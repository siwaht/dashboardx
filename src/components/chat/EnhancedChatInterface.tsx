...
import { VisualizationDemo } from './visualizations/VisualizationDemo';
...
export function EnhancedChatInterface({ sessionId, onNewSession }: EnhancedChatInterfaceProps) {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    // Check if user is asking for data analysis/visualization
    ...
    try {
      ...
      if (isDataQuery) {
        assistantResponse = `I've analyzed your data and generated a comprehensive report with visualizations. Here's what I found:`;
  ...
  const quickActions = [
    { icon: BarChart3, text: 'Analyze sales data', color: 'from-blue-500 to-cyan-500' },
    { icon: Sparkles, text: 'Show key insights', color: 'from-purple-500 to-pink-500' },
    { icon: Zap, text: 'Generate report', color: 'from-indigo-500 to-blue-500' },
    { icon: Brain, text: 'Ask a question', color: 'from-emerald-500 to-green-500' },
  ];
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center animate-fade-in">
            ...
            <p className="text-gray-600 max-w-md mb-6">
              Ask questions about your documents or data and get intelligent responses with interactive visualizations
            </p>
            <div className="grid grid-cols-2 gap-3 max-w-2xl">
              {quickActions.map((item, idx) => (
                <button
                  ...
                  className="group flex items-center gap-3 px-5 py-4 bg-white border-2 border-gray-200 rounded-xl hover:shadow-lg hover:scale-105 transition-all duration-300 hover:border-transparent"
                >
                  <div className={`p-2 bg-gradient-to-r ${item.color} rounded-lg`}>
                    <item.icon size={20} className="text-white" />
                  </div>
                  <span className="text-sm text-gray-700 group-hover:text-gray-900 font-medium text-left">
                    {item.text}
                  </span>
                </button>
              ))}
